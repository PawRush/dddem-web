...
// Manual deployment mode (without pipeline)
if (!codeConnectionArn) {
  ...
  new FrontendStack(app, `dddemFrontend-${environment}`, {
    env: { account, region },
    environment,
    buildOutputPath,
    description: `DDD East Midlands website - ${environment}`,
...
// Pipeline mode
if (codeConnectionArn) {
  new PipelineStack(app, "DddemPipelineStack", {
    env: { account, region },
    description: "CI/CD Pipeline for DDD East Midlands",
    codeConnectionArn,
    repositoryName,
    branchName,
...
cdk.Tags.of(app).add("Project", "dddem");
